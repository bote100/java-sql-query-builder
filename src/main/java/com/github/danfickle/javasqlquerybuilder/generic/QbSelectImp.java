...
import com.github.danfickle.javasqlquerybuilder.tool.Document;
...
public class QbSelectImp implements QbSelect {
    ...
    private List<QbField> m_selectFields;
    ...
    public QbSelect select(QbField... fields) {
        m_selectFields = Arrays.asList(fields);
        return this;
    }

    @Override
    public QbSelect select(String... fields) {
       m_selectFields = Arrays.stream(fields).map(QbStdFieldImp::new).collect(Collectors.toList());
       return this;
    }
    ...
    public ResultSet result() {
        ...
        try {
            return this.connection.prepareStatement(getQueryString()).executeQuery();
        } catch (SQLException throwables) {
            throwables.printStackTrace();
        }
    ...
    public List<Document> fetch() {
        ...
        try {
            ...
            while (resultSet.next()) {
                ...
                m_selectFields.forEach(field -> {
                    String fieldName = field.toString().replace("`", "");
                    try {
                        document.append(fieldName, resultSet.getObject(fieldName));
    ...
    @Override
    public QbSelect groupBy(String... fields) {
        m_groupBy = fields;
        return this;
    }
